...
local FumeOver_Red =  Class(Widget, function(self, owner)
    self.owner = owner
    Widget._ctor(self, "FumeOver_Red")
    ...
    self.over =
    {
        ...
        level = 0,
        k = 1,
        bg = self:AddChild(Image("images/poison_red_fx.xml", "fume_over.tex")),
    }
    ...
    self.top =
    {
        ...
        level = 0,
        k = 1,
        bg = self:AddChild(Image("images/poison_red_fx.xml", "fume_top.tex")),
    }
    ...
    self.inst:ListenForEvent("startfumeoverredstart", function(owner, debuff)
...
function FumeOver_Red:TurnOn(layer, intensity)
    self:StartUpdating()
    layer.base_level = .5 * (intensity or 1)
    layer.k = 5
end

function FumeOver_Red:TurnOff(layer)
    self:StartUpdating()
    layer.base_level = 0
    layer.k = 5
end
...
function FumeOver_Red:DoUpdate(layer, dt)
...
function FumeOver_Red:OnUpdate(dt)
