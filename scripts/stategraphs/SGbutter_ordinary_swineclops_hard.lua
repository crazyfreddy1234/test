...
swineclops_sg.name = "butter_ordinary_swineclops_hard"
local tuning_values = TUNING.FORGE.SWINECLOPS

---------------------------
-- Update Attack Pattern -- Swineclops can now fire projectiles at any target in midrange prior to their attack
--------------------------- Similar to Boarriors random slam
...
local stun_state = swineclops_sg.states.stun
local _oldOnEnter = stun_state.onenter
local ENRAGED_TANTRUM_COOLDOWN = 60
stun_state.onenter = function(inst, data)
    if inst.enraged and inst.tantrum_timer and not inst.sg.statemem.end_tantrum then
    ...
    if inst.enraged then
        ...
        if #targets > 0 then
            ...
            if target then
                ...
                if inst.wants_to_pigspawn and inst.wants_to_pigspawn == true then
                    inst.wants_to_pigspawn = false
                    local pig = SpawnPrefab("pitpig")
                    local pos = target:GetPosition()
                    pig.Transform:SetPosition(pos:Get())
                    pig.components.locomotor.runspeed=TUNING.FORGE.PITPIG.RUNSPEED*1.2
                    pig.components.health:SetMaxHealth(TUNING.FORGE.PITPIG.HEALTH*0.9)
                    pig.components.combat:SetDefaultDamage(TUNING.FORGE.PITPIG.DAMAGE*0.66)
                    pig.components.combat:SetAttackPeriod(TUNING.FORGE.PITPIG.ATTACK_PERIOD*1.2)
                    pig.Transform:SetScale(0.6,0.6,0.6)
                end
            end
        end
    else
...
local function RemoveAllGeysers(inst)
    ...
    if inst.wants_to_pigspawn then
        inst.wants_to_pigspawn = nil
    end
end
...
local GOTONORMAL_TANTRUM_DURATION = 10
local ENRAGED_TANTRUM_COOLDOWN = 5
...
swineclops_sg.states["enraged_tantrum"] = State{
    ...
    onenter = function(inst, data)
        ...
        if ISEXHARD or not inst.enraged then
