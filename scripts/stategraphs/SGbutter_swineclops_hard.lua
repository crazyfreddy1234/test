...
swineclops_sg.name = "butter_swineclops_hard"
...
local ENRAGED_TANTRUM_COOLDOWN = 30
...
local ENRAGED_TANTRUM_DURATION = 5
...
swineclops_sg.states["enraged_tantrum"] = State{
    ...
    onenter = function(inst, data)
        ...
        if not inst.enraged then
            inst.tantrum_timer = inst:DoTaskInTime(GOTONORMAL_TANTRUM_DURATION, function()
                inst.tantrum_timer = nil
                inst.sg.statemem.end_tantrum = true
            end)
            inst.sg:AddStateTag("nostun")
        elseif not inst.tantrum_timer then
