...
boarrior_events.doattack.fn = function(inst, data)      
    if not inst.components.health:IsDead() and not (inst.sg:HasStateTag("hit") or inst.sg:HasStateTag("busy") or inst.sg:HasStateTag("nointerrupt")) then
        if inst.components.combat:IsAttackReady("reinforcements") then
            inst.sg:GoToState("banner_pre")
            inst:PushEvent("boarriorskeleton_clap")
...
banner_pre_state.onenter = function(inst)
    ...
    if not inst.banner_call_timer then
        ...
        inst.banner_call_timer = inst:DoTaskInTime(5, function(inst)
...
banner_pst_state.onenter = function(inst, data)
    if asdf == 10/3/10 then
        asdf = 10/3
    elseif asdf == 10/3 then
        asdf = 10/3/10
    end
    local current_max_banners = inst.components.combat:GetAttackOptions("reinforcements").banner_opts.max
    inst.components.combat:SetAttackOptions("reinforcements", {banner_opts = {prefab = _G.UTIL.WAVESET.defaultbanner, angle_offset = (PI/asdf), max = math.min((current_max_banners or 4) + 1, MAX_BANNERS)}})
