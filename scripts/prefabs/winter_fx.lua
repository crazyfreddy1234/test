...
local assets_ping =
{
	...
	Asset("ANIM", "anim/bl_deerclops.zip"),
}
...
local function ping_CreateDisc()
	...
	inst.AnimState:SetBuild("bl_deerclops")
...
local function ping_OnFadeDirty(inst)
	...
	if inst.fade:value() == 1 then
		inst.disc.alpha = 1
		inst.disc.delta = 0
		inst.disc.AnimState:SetMultColour(r, g, b, 1)
	else
		inst.disc.delta = inst.fade:value() == 2 and -0.1 or 0.1
	end
end

local function OnSetColorDirty(inst)
	if inst.SetColor:value() ~= nil then
		r = flower_colors[inst.SetColor:value()][1]
		g = flower_colors[inst.SetColor:value()][2]
		b = flower_colors[inst.SetColor:value()][3]
	end
end
...
local function pingfn()
	...
	inst.fade = net_tinybyte(inst.GUID, "winter_impact_ping_fx.fade", "fadedirty")   --deerclops_icelance_ping_fx
	inst.SetColor = net_string(inst.GUID, "SetColor", "setcolordirty")

	--Dedicated server does not need to spawn the local fx
	if not TheNet:IsDedicated() then
		inst:ListenForEvent("setcolordirty", OnSetColorDirty)
...
local function impactfn()
	...
	inst.persists = false
	inst.deathtimer = inst:DoTaskInTime(2, impact_KillFX)
...
return Prefab("winter_impact_ping_fx", pingfn, assets_ping), 
	Prefab("winter_impact_circle_fx", impactfn, assets_impact)
