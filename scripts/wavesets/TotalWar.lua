...
--WAVESET DATA--
 ...
 waveset_data[1].wavemanager.onspawningfinished[1]=function(self,spawnedmobs)
	local world_center = _G.TheWorld.components.lavaarenaevent:GetArenaCenterPoint() or {x=0,y=0,z=0}
	local X_LINECOUNT = 4
	local Z_LINECOUNT = 20
	local X_STARTLOCATION = world_center.x - 6
	local X_ENDLOCATION = X_STARTLOCATION+((X_LINECOUNT-1)*4)
	local Z_STARTLOCATION = world_center.z + 34
	local Z_ENDLOCATION = Z_STARTLOCATION+((Z_LINECOUNT-1)*4)

	local function RemoveEntity(pos)
		local ents = TheSim:FindEntities(pos.x, 0, pos.z, 3) -- 반경 1로 주변 프리팹 검색

		for _, ent in ipairs(ents) do
			if not ent:HasTag("player") and ent.prefab ~= "lavaarena_spawner" then
				ent:Remove()
			end
		end
	end
	...
	local function c_setTile()
		local loopCount = 0
		for i=X_STARTLOCATION,X_ENDLOCATION,4 do
			loopCount = loopCount + 1
			for j=Z_STARTLOCATION,Z_ENDLOCATION,4 do
				...
				if loopCount == 1 or loopCount == 4 then
					_G.TheWorld.Map:SetTile(x, y, GROUND.IMPASSABLE)
					_G.TheWorld.Map:RebuildLayer(oldType, x, y)
					_G.TheWorld.Map:RebuildLayer(GROUND.IMPASSABLE, x, y)
				else
