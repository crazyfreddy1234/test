...
local TUNING = TUNING
local GetUtil,GetCommon_Fns,AddReforgedPrefab,SetCombatModifier=RPX.GetUtil,RPX.GetCommon_Fns,RPX.AddReforgedPrefab,RPX.SetCombatModifier
local ForceTaunt = GetCommon_Fns("ForceTaunt")
...
--WAVESET DATA--
 ...
 waveset_data[1].wavemanager.onspawningfinished[1]=function(self,spawnedmobs)
	for i,moblist in pairs(spawnedmobs) do 
		for mob in pairs(moblist) do 
			mob:DoTaskInTime(30, function(inst)
				mob.wants_to_tantrum = true
			end)
		end
	end
	local swineclop = _W.OrganizeAllMobs(spawnedmobs).swineclops
	self.health_triggers.swineclop={
	...
	_W.AddHealthTriggers(self.health_triggers.swineclop,unpack(swineclop))
 end
