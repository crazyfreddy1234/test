...
local function ChangeLightPhase(inst, phase, player)
    ...
    local rlgl_talker = inst:GetRLGLTalker()
    if rlgl_talker then
        local speak = phases[player_phases[player]].speech
...
local function UpdateLightPhase(inst)
    ...
    for i,player in pairs(_G.AllPlayers) do
        ...
        if player_valid_phases[player] and player_phases[player] then
    ...
    -- Queue next phase
    local goto_normal = math.random(50,90)/10
    local goto_lights = math.random(90,140)/10
    if isnormal then
        inst:DoTaskInTime(goto_lights, function()
            UpdateLightPhase(inst)
        end)
    else
        inst:DoTaskInTime(goto_normal, function()
            UpdateLightPhase(inst)
        end)
    end
...
local disco_icon = {atlas = "images/disco.xml", tex = "disco.tex"}
local rlgl_exp = {desc = "DISCOLIGHTS_WIN", val = {mult = RF_TUNING.EXP.GAMETYPES.rlgl},atlas = "images/disco.xml", tex = "disco.tex"}
_G.AddGametype("discolights", rlgl_fns, disco_icon, rlgl_exp, 10)